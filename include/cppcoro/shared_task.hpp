...
namespace cppcoro
{
	...
	namespace detail
	{
		...
		class shared_task_promise_base
		{
			...
			struct final_awaiter
			{
				...
				void await_suspend(stde::coroutine_handle<PROMISE> h) noexcept
				{
		...
		public:
			...
			stde::suspend_always initial_suspend() noexcept { return {}; }
			...
			bool try_await(shared_task_waiter* waiter, stde::coroutine_handle<> coroutine)
			{
	...
	public:
		...
		explicit shared_task(stde::coroutine_handle<promise_type> coroutine)
			: m_coroutine(coroutine)
		{
	...
	namespace detail
	{
		...
		shared_task<T> shared_task_promise<T>::get_return_object() noexcept
		{
			return shared_task<T>{
				stde::coroutine_handle<shared_task_promise>::from_promise(*this)
			};
		}
